...
```
    ...
    ## Warning: Problem with `mutate()` input `rmse_step`.
    ## ℹ prediction from a rank-deficient fit may be misleading
    ## ℹ Input `rmse_step` is `map2_dbl(step_mod, test, ~rmse(model = .x, data = .y))`.
...
is a little bit right-skewed. It is centered around 0.91 which suggests
